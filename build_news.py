...
ENRICH_SOURCES = {"industry", "voices"}  # 简报/译题只做三方外部源; 自家中文内容不需要
...
def detail_html(it):
    ...
    body = (f'<article class="dp-body">{full}</article>' if full
            else (f'<blockquote class="dp-excerpt">{esc(it["summary"])}</blockquote>' if it["summary"] else ""))
...
def write_detail_pages(vis, items):
    ...
    orphan = [p for p in CONTENT_DIR.glob("*.html") if p.stem not in keep_ids] if CONTENT_DIR.exists() else []
    for p in orphan:
        p.unlink()
