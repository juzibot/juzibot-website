...
内容源走 adapter（脚本 `SOURCES`）：`sitemap`（rui 博客逐篇抓 JSON-LD）、`rss`（RSS2/Atom 通吃 + 脏 XML 正则兜底；现有 36氪/量子位/钛媒体三路合流，feed 名落 category 与 author）、`feishu-base`（**公众号发布闸门**：飞书多维表格《官网动态发布登记》https://juzihudong.feishu.cn/base/HhPubortTafxOssddqJc4m9Znkd ，运营发文后贴公众号正式链接+勾「上官网」，管线走本机已授权的 lark-cli 只拉过闸行，og 元数据自动补齐；三个公众号——句子互动官方/AI对话未来/佳芮的创业笔记——同一张表，账号名落 category 显示为条目小标签）、`manual`（本地 JSON 投递位，备用，当前无源使用）。公众号内容的上游草稿在句子互动飞书 wiki「文章初稿」目录，但 wiki 链接访客不可见，官网只放公众号正式链接。

**AI 筛选层（2026-07-21 起）**：配了 `ai_filter` 的源走本机已登录的 claude CLI（haiku 模型）批量判定去留——`rui-blog` 规则 company 只留与公司直接相关的文章（动态页不强化创始人，个人随笔不上站），`industry` 规则 ai 只留 AI 相关资讯（股市快讯、无关融资是噪音）；公众号不筛（运营勾「上官网」本身就是人工闸门）。判定结果持久化在条目 `ai` 字段，每条只判一次；claude CLI 缺失或调用失败时新条目暂缓上站（pending），下次运行自动重试，已上站内容不受影响。页面只注入过筛条目；`data/news.json` 存全量（被筛掉的也留着，是增量去重与判定缓存的记忆）。
