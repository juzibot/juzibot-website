...
**增量缓存的版本键是 `render_ver()` = `RENDER_VER` + 模板函数源码指纹**（`_TPL_FNS` 列出的渲染函数）。改渲染模板不再需要手动 bump `RENDER_VER`——签名自己感知源码变化并触发重算，`RENDER_VER` 只留作强制全量重算的人工开关。这个机制是补窟窿补出来的：忘记 bump 导致「改了但页面没更新」已发生两次（Article schema、概念索引页文案）。
